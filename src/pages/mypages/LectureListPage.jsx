...
export default function LectureListPage() {
  ...
  const [video, setVideo] = useState({ url: null, title: "", durationSec: 0 });
  ...
  useEffect(() => {
    if (!selectedId) {
      setVideo({ url: null, title: "", durationSec: 0 });
      setVideoErr(null);
      return;
    }
  ...
  return (
    // 화면 전체 높이를 고정: 100svh (주소창 변동에 강함) + vh 폴백
    <div className="w-full min-h-screen h-[100svh] bg-white">
      {/* 좌우 2열 비율 고정: 좌 7fr / 우 3fr, 우측 최소폭 320px */}
      <div
        className="mx-auto h-full px-3 sm:px-4 lg:px-6 py-3 grid gap-4
                      grid-cols-[minmax(0,7fr)_minmax(320px,3fr)]"
      >
        {/* LEFT: 플레이어 - 부모 높이 꽉 채움 */}
        <section className="h-full">
          <div className="h-full rounded-2xl border bg-black overflow-hidden flex items-center justify-center">
            {videoLoading && (
              <div className="text-white/80 text-sm">영상 로딩 중…</div>
            )}
            {!videoLoading && videoErr && (
              <div className="text-red-300 text-sm">{videoErr}</div>
            )}
        ...
        {/* RIGHT: 목차 패널 - 부모 높이 꽉 채움, 내부 스크롤 */}
        <aside className="h-full">
          <div className="h-full rounded-2xl border bg-white flex flex-col overflow-hidden">
            {/* 상단 고정 헤더 */}
            <div className="sticky top-0 z-10 bg-white border-b">
              <div className="p-3 sm:p-4 flex items-center justify-between gap-2">
                ...
                <div className="text-base sm:text-lg font-bold">강의 목차</div>
              </div>
                    ...
                    <div className="shrink-0">
                      <button
                        ...
                        className={[
                          ...
                          "hover:bg-gray-50",
                          l.__isActive ? "border-rose-500 text-rose-600" : "",
                        ].join(" ")}
